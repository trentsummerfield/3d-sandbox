...
layout(location = 3) uniform float now;
...
void main()
{
    float disolve_time = now * 0.5;
    if (fract(disolve_time / (2.0 * PI)) < 0.5) {
        disolve_time += PI;
    }
    float noise = snoise(v_position * 0.5);
    float threshold = cos(disolve_time) * 1.1;
    if (noise < threshold) {
        discard;
    }
    ...
    color += vec3(smoothstep(-0.8, 1.0, threshold), smoothstep(0.0, 1.0, threshold), smoothstep(0.2, 1.0, threshold));
    if (noise - threshold < 0.05) {
        float dist = smoothstep(0.0, 0.1, noise - threshold);
        color = color + 2.0 * vec3(1.0, dist, dist);
    }
